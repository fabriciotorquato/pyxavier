...
from .packet import EmotivExtraPacket, EmotivNewPacket, EmotivOldPacket
...
from .util import is_extra_data, path_checker, system_platform, values_header
        ...
        if self.write:
            ...
            if self.write_decrypted:
                ...
                if not self.read_values:
                    output_path = 'values_%s.csv' % str(datetime.now()).replace(':', '-')
                    if self.output_path is not None:
                        if type(self.output_path) == str:
                            output_path = path_checker(self.output_path, output_path)
